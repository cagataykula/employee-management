...
describe('EmployeeList', () => {
	...
	it('should handle bulk delete cancellation', async () => {
		...
		element._selectedIds = new Set(['1', '2'])
		element._showBulkDeleteModal = true
		await element.updateComplete
		...
		expect(element._showBulkDeleteModal).toBe(false)
	})

	it('should handle row selection deselection', async () => {
		element = await fixture(
			html`
				<employee-list></employee-list>
			`
		...
		// Initially select an employee
		element._selectedIds = new Set(['1'])
		await element.updateComplete

		expect(element._selectedIds.has('1')).toBe(true)

		// Call the handler directly
		element._handleRowSelectionChange({
			detail: { employeeId: '1', isChecked: false }
		})
		await element.updateComplete

		expect(element._selectedIds.has('1')).toBe(false)
	})

	it('should show no results message in list view when no employees match search', async () => {
		// Set empty employees array
		_mockGetEmployees.mockReturnValue([])
		element = await fixture(
			html`
				<employee-list></employee-list>
			`
		...
		const noResultsMessage = element.shadowRoot.querySelector('.no-results')
		expect(noResultsMessage).toBeDefined()
		expect(noResultsMessage.textContent).toContain('No employees') // Check for English no results message
	})

	it('should handle language change and request update', async () => {
		element = await fixture(
			html`
				<employee-list></employee-list>
			`
	...
	it('should properly clean up event listeners on disconnect', async () => {
		...
		element._unsubscribe = mockUnsubscribe
	...
	it('should handle page navigation edge cases', async () => {
		// Set up larger dataset for pagination
		...
		_mockGetEmployees.mockReturnValue(largeDataset)
		element = await fixture(
			html`
				<employee-list></employee-list>
			`
		...
		// Test total pages calculation first
		const totalPages = element._getTotalPages()
		expect(totalPages).toBe(3) // 20 items / 8 per page = 3 pages max

		// Test going to page beyond available pages
		element._goToPage(10) // Should clamp to last page or stay within bounds
		await element.updateComplete

		expect(element._currentPage).toBeLessThanOrEqual(3) // Should not exceed total pages

		// Test going to page 0 or negative
		element._goToPage(0)
		await element.updateComplete

		expect(element._currentPage).toBeGreaterThanOrEqual(1) // Should not go below 1
	})

	it('should handle select all checkbox states correctly', async () => {
		element = await fixture(
			html`
				<employee-list></employee-list>
			`
		...
		// Test with no employees selected
		const noneState = element._getHeaderCheckboxState()
		expect(noneState.checked).toBe(false)
		expect(noneState.indeterminate).toBe(false)

		// Test with all employees selected
		const currentPageEmployees = element._getCurrentPageEmployees()
		element._selectedIds = new Set(currentPageEmployees.map((emp) => emp.id))
		await element.updateComplete

		const allState = element._getHeaderCheckboxState()
		expect(allState.checked).toBe(true)

		// Test with some employees selected
		element._selectedIds = new Set([currentPageEmployees[0].id])
		await element.updateComplete

		const someState = element._getHeaderCheckboxState()
		expect(someState.indeterminate).toBe(true)
	})

	it('should handle filtered employees getter properly', async () => {
		element = await fixture(
			html`
				<employee-list></employee-list>
			`
		...
		element._searchTerm = ''
		const allEmployees = element._filteredEmployees
		expect(allEmployees).toHaveLength(mockEmployees.length)

		// Test with search term
		element._searchTerm = 'First1'
		const filteredEmployees = element._filteredEmployees
		expect(filteredEmployees.length).toBeLessThan(allEmployees.length)
		expect(
			filteredEmployees.every(
				(emp) =>
					emp.firstName.includes('First1') ||
					emp.lastName.includes('First1') ||
					emp.email.includes('First1')
			)
		).toBe(true)
	})
	...
	it('should handle modal close events correctly', async () => {
		...
		// Test single delete modal close by calling handler directly
		element._showDeleteModal = true
		element._employeeToDeleteId = '1'
		await element.updateComplete

		element._closeDeleteModal()
		await element.updateComplete

		expect(element._showDeleteModal).toBe(false)
		expect(element._employeeToDeleteId).toBe(null)

		// Test bulk delete modal close by calling handler directly
		element._showBulkDeleteModal = true
		element._selectedIds = new Set(['1', '2'])
		await element.updateComplete

		element._closeBulkDeleteModal()
		await element.updateComplete
