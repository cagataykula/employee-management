...
// Expected formatted date (DD/MM/YYYY)
const expectedFormattedDate = '15/10/1843'
...
describe('EmployeeCard', () => {
	...
	it('should format the date correctly (DD/MM/YYYY)', async () => {
		const el = await fixture(
			html`
				<employee-card .employee=${mockEmployee}></employee-card>
			`
		...
		const el2 = await fixture(
			html`
				<employee-card .employee=${employeeWithNullDate}></employee-card>
			`
		)
		...
		const el3 = await fixture(
			html`
				<employee-card .employee=${employeeWithInvalidDate}></employee-card>
			`
		)
	...
	it('should render empty if employee data lacks an id', async () => {
		...
		const el = await fixture(
			html`
				<employee-card .employee=${employeeWithoutId}></employee-card>
			`
		)
		expect(el.shadowRoot.querySelector('.employee-card')).toBeNull()
	})
