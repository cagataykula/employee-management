...
import '../ui/confirmation-modal.js' // Import the modal
...
class EmployeeForm extends LitElement {
	...
	_validateForm() {
		...
		if (!isRequired(this._firstName)) {
			errors.push(`${t('firstName')}: ${t('fieldRequired')}`)
		} else if (!hasMinLength(this._firstName, 2)) {
			errors.push(`${t('firstName')}: ${t('minLengthError', { min: 2 })}`)
		}

		if (!isRequired(this._lastName)) {
			errors.push(`${t('lastName')}: ${t('fieldRequired')}`)
		} else if (!hasMinLength(this._lastName, 2)) {
			errors.push(`${t('lastName')}: ${t('minLengthError', { min: 2 })}`)
		}

		// Email validation
		if (!isRequired(this._email)) {
			errors.push(`${t('emailAddress')}: ${t('fieldRequired')}`)
		} else if (!isValidEmail(this._email)) {
			errors.push(`${t('emailAddress')}: ${t('invalidEmail')}`)
		}

		// Phone validation
		if (!isRequired(this._phoneNumber)) {
			errors.push(`${t('phoneNumber')}: ${t('fieldRequired')}`)
		} else if (!isValidPhone(this._phoneNumber)) {
			errors.push(`${t('phoneNumber')}: ${t('invalidPhone')}`)
		}

		// Date validations
		if (!isRequired(this._dateOfEmployment)) {
			errors.push(`${t('dateOfEmployment')}: ${t('fieldRequired')}`)
		} else if (!isValidDate(this._dateOfEmployment)) {
			errors.push(`${t('dateOfEmployment')}: ${t('invalidDate')}`)
		}

		if (!isRequired(this._dateOfBirth)) {
			errors.push(`${t('dateOfBirth')}: ${t('fieldRequired')}`)
		} else if (!isValidDate(this._dateOfBirth)) {
			errors.push(`${t('dateOfBirth')}: ${t('invalidDate')}`)
		} else if (!isNotFutureDate(this._dateOfBirth)) {
			errors.push(`${t('dateOfBirth')}: ${t('futureDateNotAllowed')}`)
		}

		// Department and position (required)
		if (!isRequired(this._department)) {
			errors.push(`${t('department')}: ${t('fieldRequired')}`)
		}

		if (!isRequired(this._position)) {
			errors.push(`${t('position')}: ${t('fieldRequired')}`)
		}
